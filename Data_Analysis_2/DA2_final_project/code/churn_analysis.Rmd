...
Our exploratory data analysis shows that around 10% of the of total customers have churned (**Figure 2**). All Consumption related variables (cons\_12m ,cons\_last\_month,imp\_conscons\_gas\_12m ) and 2 of the forecast variables (forecast\_cons\_12m,forecast\_meter\_rent\_12m) are extremely right skewed (**Figure 3,4**).The values on the higher end and lower ends of the distribution are potentially outliers. Next we checked a non-parametric estimator lowess smoother to have a general idea about the functional form between a variable and churn. We will specifically focus on those variables who had skewed distributions (Figure . As expect the functional forms are very non linear and hence we took log of the 6 variables( 4 consumption and 2 forecast variables) to give the loess line a more linear shape (**Figure 8**).
...
ggpubr::ggarrange(forecast_meter_rent_12m_sc,ln_forecast_meter_rent_12m_sc, nrow = 1, ncol = 2)
