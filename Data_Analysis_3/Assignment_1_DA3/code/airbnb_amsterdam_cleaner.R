...
# Split the amenities into separate binary columns
...
data <- data %>% mutate(
  "Mini frige" = "Mini fridge",
  "Nespresso coffee machine" = "Nespresso machine",
  "Shower gel soap" = "Shower gel",
  "BBQ utensils" = "Barbecue utensils",
  "Freezer frige" = "Freezer",
  "free garage parking" = "` Free residential garage on premises`",
  "Mini frige" = "Mini fridge"
)
...
data <- data %>% select(-(25:152))
data <- merge(data,amenities_clean, by = "id", all = TRUE)
...
data <- data %>% select(-c("amenities", "Babysitter recommendations", "Baby bath", "Baking sheet", "` Children\u2019s dinnerware`"))

names(data)[names(data) == "frige_agg"] <- "Refridgerator"
